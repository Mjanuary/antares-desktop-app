...
import { calculateSalesData, DiversSaleItemType_Details } from "./sale.utils";
...
export default function SaleForm({ saleId }: MyComponentProps) {
  ...
  const productsList = saleFormData?.productsList || [];
  ...
  const [selectedCurrency, setSelectedCurrency] = useState<CurrencyEnum | null>(
    (branch?.branch_currency as CurrencyEnum) || null,
  );

  const [payedDolRaw, setPayedDolRaw] = useState<string>("");
  const [payedFcRaw, setPayedFcRaw] = useState<string>("");
  const [payedFrwRaw, setPayedFrwRaw] = useState<string>("");
  ...
  useEffect(() => {
    ...
    const rate = branchRate as BranchRate;
  ...
  const payedDol = getNumber(payedDolRaw || 0);
  const payedFc = getNumber(payedFcRaw || 0);
  const payedFrw = getNumber(payedFrwRaw || 0);
  ...
  const createSalePending = false || saleSuccess;
      ...
      <div className="flex-1 flex lg:flex-row flex-col">
        ...
        >
          ...
          <div className="px-6 pt-4 grid lg:grid-cols-3 gap-x-6 gap-y-3 pb-3">
            <FindCreateClient
              onSelectClient={({ names, id, phone_number }) => {
                setClientName(names);
                  ...
                  {supportedPayments?.includes(CurrencyEnum.USD) && (
                    ...
                    >
                      <Input
                        type="text"
                        value={payedDolRaw}
                        onChange={(e) => setPayedDolRaw(e.target.value)}
                        inputSize="xl"
                        label="USD"
                        error={
                  ...
                  {supportedPayments?.includes(CurrencyEnum.RWF) && (
                    ...
                    >
                      <Input
                        ...
                        label="RWF"
                        value={payedFrwRaw}
                        onChange={(e) => setPayedFrwRaw(e.target.value)}
                        error={
                  ...
                  {supportedPayments?.includes(CurrencyEnum.CDF) && (
                    ...
                    >
                      <Input
                        ...
                        label="CDF"
                        value={payedFcRaw}
                        onChange={(e) => setPayedFcRaw(e.target.value)}
                        error={
        ...
        <ProductSellItems
          ...
          setProductsList={(products) => setProductsList(saleId, products)}
          disabled={createSalePending}
        />
      </div>

      <div
        className="fixed bottom-0 right-0 left-0 bg-gray-600/30 backdrop-blur-lg border-t border-color-theme p-2 flex justify-end"
        style={{
          left: "72px",
        }}
      >
