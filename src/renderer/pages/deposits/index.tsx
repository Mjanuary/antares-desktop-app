...
import { DepositFilterModal } from "../components/deposit-filter-modal";
...
interface DepositType {
  ...
  total_bc: number;
  decision: string;
  // approved: boolean; // decision is more specific
  row_deleted: any;
}
...
const DepositsPage = () => {
  ...
  const fetchDeposits = async () => {
    ...
    try {
      if (!window.electronAPI.getDeposits) {
        console.error("getDeposits API not found");
        return;
      }
      const response = await window.electronAPI.getDeposits(
        page,
        pageSize,
        searchText,
        {
  ...
  const columns: ColumnType<DepositType>[] = [
    {
      key: "created_date",
      label: t("common.date"),
      render: (row) => (
    ...
    },
    {
      ...
      render: (row) => {
        let variant:
          ...
          | null
          | undefined = "outline";
        if (row.decision === "APPROVED") variant = "default";
        if (row.decision === "REJECTED") variant = "destructive";

        return <Badge variant={variant}>{row.decision || "PENDING"}</Badge>;
      },
    },
  ];
      ...
      <div className="p-6 h-full flex flex-col gap-4 text-white">
        <div className="flex-1 overflow-auto">
          <CustomTable
            ...
            header={{
              title: t("deposits.page_title", "Deposits"),
              description: t(
                "deposits.page_description",
            ...
            filtersDisplay={[
              ...
              {
                key: "decision",
                text: `Status: ${filters.decision}`,
                showClose: true,
                visible: !!filters.decision,
              },
            ]}
          />
        </div>
